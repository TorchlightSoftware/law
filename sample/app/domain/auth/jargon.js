const redisId = /[a-z0-9]{16}/;
const mongoId = /[a-f0-9]{24}/;

module.exports = [{
    typeName: 'String',
    validation({value, fieldName}, assert) {
      assert(typeof value === 'string', {message: `${fieldName} is not a string.`});
    },
  ...
  , {
    ...
    validation({value}, assert) {
      assert(value && (typeof value === 'string') && value.match(redisId));
    },
  ...
  , {
    ...
    validation({value}, assert) {
      assert(value && (typeof value === 'string') && value.match(mongoId));
    },
