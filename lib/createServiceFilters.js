...
const generateValidations = function(serviceName, name, types, required) {
  ...
  // get lookups
  addTo(stack, types.map(function(t) {
    if (t.lookup) {
  ...
  // check existence
  addTo(stack, generateFilter(`${name}.exists`, function(args, next) {
  ...
  addTo(stack, types.map(function(t) {
    if (t.validation) {
      return generateFilter(`${name}.isValid(${t.typeName})`, function(args, next) {
  ...
  jargon =>
    ...
    function(serviceName, fieldNames, required) {
      ...
      for (var name of fieldNames) {
        const types = jargon
          .filter((word) => word.defaultArgs
            && word.defaultArgs.includes(name)
          );
