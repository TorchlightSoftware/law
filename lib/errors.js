...
class InvalidArgumentError extends LawError {
  constructor(context, start) {
    ...
    const message = context.message || `'${serviceName}' requires '${fieldName}' to be a valid '${requiredType}'.`;
...
InvalidArgumentsObjectError.prototype.name = 'LawError/InvalidArgumentsObjectError'
...
class NoFilterArrayError extends LawError {
  constructor(context, start) {
    const message = "Error loading policy: Validations must contain array of filters.";
