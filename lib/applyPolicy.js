...
const getPolicy = function(services, policy) {
  ...
  const validateServices = serviceNames => {
    ...
    for (let name of serviceNames) {
      const context =
        {serviceName: name};
      if (!services[name]) { throw (new InvalidServiceNameError(context)); } else {
  ...
  for (let rule of policy.rules) {
    var service;
    const context = {rule};
    if (rule.filters == null) { throw new NoFilterArrayError(context); }
    ...
    if (rule.only != null) {
      ...
      for (service in services) {
        ...
        if (policy.filterPrefix && startsWith(service, policy.filterPrefix)) continue
